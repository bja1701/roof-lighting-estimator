...
function resolveAddressForJob(
	...
	if (formattedFromSearch?.trim())
		return Promise.resolve(formattedFromSearch.trim());
...
export default function SaveToJobModal({ onSaved, onClose }: Props) {
	...
	const handleSave = async () => {
		if (atLimit) return;
		if (mode === 'existing' && !selectedJobId) {
			setError('Select a job');
			return;
		}
		if (mode === 'new' && !newJobName.trim()) {
		...
		if (!ensured.ok) {
			...
			setError(
				ensured.error ??
					'Your account profile is still syncing. Refresh and try again.'
			);
			return;
		}
		...
		const jobAddress = await resolveAddressForJob(
			estimateSiteAddress,
			satelliteCenter
		);
					...
					{atLimit ? (
						<div className="text-center py-6">
							...
							<p
								...
								style={{ color: 'var(--color-slate)' }}
							>
								You've used all 5 free estimates. Contact us to upgrade your
								account.
							</p>
							<button
								type="button"
