...
const MemoizedStreetView = memo(
	({ position }: MemoizedStreetViewProps) => {
		const [pov, setPov] = useState<{ heading: number; pitch: number }>({
			heading: 0,
			pitch: 10,
		});
		...
		useEffect(() => {
			...
			svService.getPanorama(
				{ location: targetLoc, radius: 50 },
				(data: any, status: any) => {
					if (status === 'OK' && data?.location?.latLng) {
						const carLoc = data.location.latLng;
						const heading = win.google.maps.geometry.spherical.computeHeading(
							carLoc,
							targetLoc
						);
...
const VisualPitchTool: React.FC = () => {
	const streetViewPosition = useEstimatorStore(
		(state) => state.streetViewPosition
	);
	const selectedLineId = useEstimatorStore((state) => state.selectedLineId);
	const selectedLine = useEstimatorStore((state) =>
		state.lines.find((l) => l.id === selectedLineId)
	);
		...
		>
			...
			>
				...
				>
					...
					<div
						className="text-2xl font-semibold leading-none sm:mt-1"
				...
				{/* Apply control */}
				<div
					className="flex items-center gap-3 rounded-lg px-3 py-2 sm:flex-col sm:items-stretch sm:justify-between"
					style={{
						background: 'rgba(255,255,255,0.04)',
						border: '1px solid rgba(255,255,255,0.06)',
					}}
				>
					<div
						className="min-w-0 flex-1 text-[11px] sm:flex-none"
