...
import {
	hasProAccess,
	isFreeTierEstimatorExhausted,
} from '../utils/estimatorAccess';
					...
					{NAV_ITEMS.map(({ label, Icon, path }) => {
						...
						return (
							<button
								...
								title={
									locked
										? 'Free estimate limit reached (5/5). Upgrade to continue.'
										: undefined
								}
								onClick={() => {
				...
				{NAV_ITEMS.map(({ label, Icon, path }) => {
					...
					return (
						<button
							...
							style={{
								color: active ? 'var(--color-primary)' : 'var(--color-slate)',
							}}
						>
