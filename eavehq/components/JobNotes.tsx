...
import type { JobAttachment, JobNote } from '../types/client';

const GENERATE_SIGNED_URL =
	'https://bsbewwwflqjlxxovjgec.supabase.co/functions/v1/generate-signed-url';
...
function NoteCard({
	...
	const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (storageError) {
			setUploadError(
				`Upload failed: ${storageError.message}. Note text is saved.`
			);
			setUploading(false);
			return;
		}
		...
		if (attachError || !attachData) {
			setUploadError(
				`Saved file but failed to record attachment: ${attachError?.message ?? 'unknown error'}`
			);
			return;
		}
