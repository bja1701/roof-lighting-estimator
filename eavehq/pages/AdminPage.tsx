...
export default function AdminPage() {
  ...
  const [tierFilter, setTierFilter] = useState<'all' | UserRow['subscription_status']>('all');
  ...
  const fetchData = async () => {
    ...
    const [{ data: userData }, { data: fbData }] = await Promise.all([
      ...
      supabase.from('feedback').select('*, profiles:user_id(full_name, email, company_name)').order('created_at', { ascending: false }),
    ]);
    setUsers(userData ?? []);
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesTier = tierFilter === 'all' || user.subscription_status === tierFilter;
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-10 py-10">
        ...
        <div className="flex items-center justify-between mb-12">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: 'var(--color-slate)' }}>User management, tier control, and feedback inbox.</p>
          </div>
          <span
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5 mb-10">
          {[
            ...
            { label: 'Free Tier', value: stats.free, color: 'var(--color-slate)' },
            { label: 'Active', value: stats.active, color: 'var(--color-success)' },
            { label: 'Canceled', value: stats.canceled, color: 'var(--color-warning)' },
          ].map(s => (
            <div
              key={s.label}
            ...
            >
              <p className="text-[10px] font-semibold uppercase tracking-widest mb-2" style={{ color: 'var(--color-slate)' }}>{s.label}</p>
        ...
        {/* Tabs */}
        <div className="flex p-1 rounded-lg w-fit mb-6" style={{ background: 'var(--color-surface)' }}>
          {(['users', 'feedback'] as const).map(t => (
            <button
              ...
              style={{
                ...
                color: tab === t ? 'var(--color-primary)' : 'var(--color-slate)',
              }}
            >
              {t}
            </button>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-16" style={{ color: 'var(--color-slate)' }}>
          ...
          <div className="space-y-4">
            ...
            >
                ...
                <thead style={{ background: 'var(--color-surface)' }}>
                  <tr>
                    {['User', 'Company', 'Tier', 'Estimates', 'Joined'].map(h => (
                      <th key={h} className="text-left px-5 py-3.5 text-[10px] font-bold uppercase tracking-wider" style={{ color: 'var(--color-slate)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {filteredUsers.map(u => {
                    const estimatePct = Math.min(100, ((u.estimates_used ?? 0) / 5) * 100);
                    return (
                      <tr key={u.id} className="border-t transition-colors" style={{ borderColor: 'var(--color-border)' }}>
                        <td className="px-5 py-4">
                          <p className="font-medium" style={{ color: 'var(--color-ink)' }}>{u.full_name ?? '—'}</p>
                          <p className="text-xs" style={{ color: 'var(--color-slate)' }}>{u.email ?? '—'}</p>
                        </td>
                        <td className="px-5 py-4" style={{ color: 'var(--color-slate)' }}>{u.company_name ?? '—'}</td>
                        <td className="px-5 py-4">
                          <select
                        ...
                        <td className="px-5 py-4">
                          <div className="min-w-[120px]">
                            <div className="mb-1 flex items-center justify-between gap-2 text-xs" style={{ color: 'var(--color-slate)' }}>
                              <span style={{ fontFamily: 'var(--font-mono)' }}>{u.estimates_used ?? 0}/5</span>
                              {u.subscription_status === 'active' && <span>Pro</span>}
                            </div>
                            <div className="h-1.5 overflow-hidden rounded-full" style={{ background: 'var(--color-surface)' }}>
                              <div
                                ...
                                style={{
                                  width: `${estimatePct}%`,
                                  background: estimatePct >= 80 ? 'var(--color-warning)' : 'var(--color-primary)',
                                }}
                              />
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-xs" style={{ color: 'var(--color-slate)' }}>
              ...
              {filteredUsers.length === 0 && <p className="text-center py-10" style={{ color: 'var(--color-slate)' }}>No users match that filter.</p>}
            </div>
          </div>
        ) : (
          <div className="space-y-4">
            {feedback.length === 0 && <p className="text-center py-10" style={{ color: 'var(--color-slate)' }}>No feedback yet.</p>}
            {feedback.map(f => (
              <div
                key={f.id}
                className="rounded-xl p-6"
                    ...
                    <div className="mb-1 flex items-center gap-0.5">
                      {f.rating && [1,2,3,4,5].map(s => (
                        <Star
                          key={s}
                          size={18}
                          fill={s <= f.rating! ? 'var(--color-primary)' : 'none'}
                          style={{ color: s <= f.rating! ? 'var(--color-primary)' : 'var(--color-border)' }}
                        />
                      ))}
                    </div>
                    <p className="text-xs" style={{ color: 'var(--color-slate)' }}>
                      {f.profiles?.full_name ?? f.profiles?.email ?? 'Unknown user'}
                      {f.profiles?.company_name ? ` · ${f.profiles.company_name}` : ''}
                    </p>
                  </div>
                  <span className="text-xs" style={{ color: 'var(--color-slate)' }}>{new Date(f.created_at).toLocaleDateString()}</span>
                </div>
                <p className="text-sm" style={{ color: 'var(--color-ink)' }}>{f.message}</p>
                {f.page && <p className="text-xs mt-1.5" style={{ color: 'var(--color-slate)' }}>Page: {f.page}</p>}
              </div>
            ))}
          </div>
        )}
