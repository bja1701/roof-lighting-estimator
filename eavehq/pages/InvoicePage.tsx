...
export default function InvoicePage() {
  ...
  return (
    <>
      ...
      <div className="min-h-screen py-10 px-4" style={{ background: 'var(--color-surface)', fontFamily: 'var(--font-body)' }}>
        {/* Print button */}
        <div className="max-w-3xl mx-auto mb-6 flex justify-end no-print">
          <button
            ...
            style={{ background: 'var(--color-accent)', color: '#fff', boxShadow: '0 2px 8px rgba(217,111,10,0.3)' }}
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-xl font-bold leading-tight" style={{ fontFamily: 'var(--font-display)' }}>
                    {contractorName}
                  </p>
                  <p className="text-sm mt-0.5" style={{ color: 'rgba(255,255,255,0.5)' }}>Estimate Invoice</p>
                  {contractor?.email && (
                    <p className="text-sm mt-0.5" style={{ color: 'rgba(255,255,255,0.5)' }}>{contractor.email}</p>
                  )}
                  {contractor?.phone && (
                    <p className="text-sm mt-0.5" style={{ color: 'rgba(255,255,255,0.5)' }}>{contractor.phone}</p>
                  )}
                </div>
              </div>
              <div className="text-right text-sm flex-shrink-0" style={{ color: 'rgba(255,255,255,0.5)' }}>
                <p className="text-white font-bold text-lg" style={{ fontFamily: 'var(--font-display)' }}>{job.name}</p>
          ...
          <div className="px-8 py-8 space-y-8">
            {quotes.length === 0 ? (
              <p className="text-sm text-center py-8" style={{ color: 'var(--color-slate)' }}>No estimates available.</p>
            ) : (
              quotes.map((quote, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div>
                        <span className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--color-slate)' }}>
                          Option {idx + 1}
                        </span>
                        <h2 className="text-lg font-bold mt-0.5" style={{ color: 'var(--color-ink)', fontFamily: 'var(--font-display)' }}>
                          {quote.label}
                        </h2>
                      </div>
                    ...
                    <div className="px-6 py-5 space-y-4">
                      ...
                      {lineItems.length > 0 && (
                        <div>
                          <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: 'var(--color-slate)' }}>
                            Line Items
                          </p>
                          <table className="w-full text-sm">
                            <thead>
                              <tr style={{ borderBottom: '1px solid var(--color-border)' }}>
                                {['Type', 'Pitch', 'Length (ft)', 'Cost'].map((h, i) => (
                                  <th
                                    key={h}
                            ...
                            <tbody>
                              {lineItems.map((item) => (
                                <tr key={item.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                                  <td className="py-1.5 capitalize" style={{ color: 'var(--color-ink)' }}>{item.type}</td>
                                  <td className="py-1.5" style={{ color: 'var(--color-slate)' }}>{item.pitch}</td>
                                  <td className="py-1.5 text-right" style={{ color: 'var(--color-ink)', fontFamily: 'var(--font-mono)' }}>
                      ...
                      >
                        <div className="text-sm">
                          <span style={{ color: 'var(--color-slate)' }}>Total linear ft: </span>
                          <span className="font-semibold" style={{ color: 'var(--color-ink)', fontFamily: 'var(--font-mono)' }}>
                            {quote.total_linear_ft != null ? `${quote.total_linear_ft.toFixed(1)} ft` : '—'}
                          </span>
                          {quote.price_per_foot != null && (
                            <span className="ml-2" style={{ color: 'var(--color-slate)', fontFamily: 'var(--font-mono)' }}>
                        ...
                        <div className="text-sm text-right">
                          <span style={{ color: 'var(--color-slate)' }}>Deposit ({job.deposit_percent}%): </span>
                          <span className="font-bold" style={{ color: 'var(--color-ink)', fontFamily: 'var(--font-mono)' }}>
          ...
          >
            <p className="text-sm text-center" style={{ color: 'var(--color-slate)' }}>
              Questions? Contact{' '}
              <span className="font-semibold" style={{ color: 'var(--color-ink)' }}>{contractorName}</span>
            </p>
          </div>
        </div>
      </div>
