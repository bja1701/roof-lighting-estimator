...
export default function ClientPortalPage() {
	...
	const fetchPortalData = async (portalToken: string) => {
		...
		const { data: jobData, error: jobError } = await supabase
			.from('jobs')
			.select(
				'id, user_id, address, status, deposit_percent, deposit_amount, deposit_paid_at, portal_token'
			)
		...
		const [{ data: quotesData }, { data: profileData }] = await Promise.all([
			supabase
				.from('quotes')
				.select(
					'id, label, total_linear_ft, total_price, notes, discount_amount, discount_type'
				)
		...
		if (loadedQuotes.length > 0) {
			const preSelected =
				...
				loadedQuotes.some((q) => q.id === deepLinkedEstimateId)
					? deepLinkedEstimateId
					: loadedQuotes[0].id;
			...
			>
				<div className="text-center max-w-sm">
					...
					<p className="text-sm" style={{ color: 'var(--color-slate)' }}>
						This quote link is no longer valid. Please contact your contractor
						for an updated link.
					</p>
				</div>
			</div>
	...
	return (
		...
		>
			...
			<main className="max-w-lg mx-auto px-4 py-8 space-y-6">
				...
				{isFinalPaid && (
					<SuccessBanner>
						<p className="font-bold" style={{ color: '#1a5c38' }}>
				...
				{isReadyForFinal && !isFinalPaid && (
					<>
						...
						<div>
							...
							<p className="text-sm" style={{ color: 'var(--color-slate)' }}>
								Your installation is complete. Please pay the remaining balance
							...
							<PayCard>
								{finalDollars != null && (
									<div className="flex items-center justify-between text-sm">
										...
										>
											$
											{finalDollars.toLocaleString('en-US', {
				...
				{!isDepositPaid && (
					<>
						<div>
							...
							<p className="text-sm" style={{ color: 'var(--color-slate)' }}>
								Select the option that works best for you, then pay your deposit
							...
							<div className="space-y-3">
								{quotes.map((quote) => {
									...
									const hasDiscount =
										effectivePrice != null &&
										quote.total_price != null &&
										effectivePrice < quote.total_price;
										...
										<button
											...
											style={{
												background: isSelected
													? 'rgba(217,111,10,0.05)'
													: 'var(--color-card)',
												border: `2px solid ${isSelected ? 'var(--color-accent)' : 'var(--color-border)'}`,
												padding: '20px',
												boxShadow: isSelected
													? '0 2px 8px rgba(217,111,10,0.12)'
													: undefined,
											}}
											onMouseEnter={(e) => {
												if (!isSelected)
												...
												<div className="flex items-center gap-3">
													...
													<div>
														...
														{quote.total_linear_ft != null && (
															...
															>
																{Math.round(
																	quote.total_linear_ft
																).toLocaleString()}{' '}
																linear ft
															</p>
														)}
														{job.deposit_percent != null &&
															effectivePrice != null && (
																<p
																	className="text-sm mt-0.5"
																	style={{ color: 'var(--color-slate)' }}
							...
							<PayCard>
								{depositDollars != null && (
									<div className="flex items-center justify-between text-sm">
										...
										>
											$
											{depositDollars.toLocaleString('en-US', {
												minimumFractionDigits: 2,
												maximumFractionDigits: 2,
											})}
										</span>
