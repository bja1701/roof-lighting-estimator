...
import { Upload, Search, Users, ChevronRight, X, AlertCircle, CheckCircle2 } from 'lucide-react';
...
export default function ClientsPage() {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (emailsToCheck.length > 0) {
      const { data } = await supabase.from('clients').select('*').in('email', emailsToCheck);
      (data ?? []).forEach((c: Client) => { existingByEmail[c.email!.toLowerCase()] = c; });
    }
  ...
  const runImport = async (
    ...
    for (const row of cleanRows) {
      ...
      if (error) { errors.push(`Failed to import "${row.name}": ${error.message}`); }
      else       { successCount++; }
    }
    ...
    for (const mc of merges) {
      ...
      if (resolution === 'merge') {
        ...
        if (error) { errors.push(`Merge failed for "${mc.incoming.name}": ${error.message}`); }
        else       { successCount++; }
      } else {
        const { error } = await supabase.from('clients').insert({
          contractor_id:  user.id,
        ...
        if (error) { errors.push(`Failed to import "${mc.incoming.name}": ${error.message}`); }
        else       { successCount++; }
      }
    }
  ...
  return (
    <SharedLayout>
      <div style={{ maxWidth: '48rem', margin: '0 auto', padding: '32px 20px 48px' }}>
        ...
        <div className="flex items-start justify-between gap-4" style={{ marginBottom: '24px' }}>
          ...
          <button
            ...
            onMouseEnter={e => { if (!importing) { const b = e.currentTarget; b.style.background = 'var(--color-surface)'; b.style.color = 'var(--color-ink)'; b.style.borderColor = 'var(--color-slate)'; } }}
            onMouseLeave={e => { const b = e.currentTarget; b.style.background = 'transparent'; b.style.color = 'var(--color-slate)'; b.style.borderColor = 'var(--color-border)'; }}
          >
          ...
          {search && (
            <button
              ...
              style={{
                ...
                color: 'var(--color-slate)', padding: '4px',
        ...
        {showImportResult && (
          <div
            ...
            style={{
              ...
              border: importErrors.length > 0
                ? '1px solid rgba(201,64,64,0.3)'
                : '1px solid rgba(61,158,106,0.3)',
              background: importErrors.length > 0
                ? 'rgba(201,64,64,0.05)'
                : 'rgba(61,158,106,0.05)',
            }}
          >
            {importErrors.length > 0
              ? <AlertCircle size={16} style={{ color: 'var(--color-destructive)', flexShrink: 0, marginTop: '1px' }} />
              : <CheckCircle2 size={16} style={{ color: 'var(--color-success)', flexShrink: 0, marginTop: '1px' }} />
            }
            <div style={{ flex: 1 }}>
              <p style={{ margin: 0, fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontWeight: 600, color: 'var(--color-ink)' }}>
                {importSuccess} record{importSuccess !== 1 ? 's' : ''} imported.
              </p>
              {importErrors.length > 0 && (
                <ul style={{ margin: '8px 0 0', padding: 0, listStyle: 'none' }}>
                  {importErrors.map((err, i) => (
                    <li key={i} style={{ fontFamily: 'var(--font-body)', fontSize: '0.75rem', color: 'var(--color-destructive)', marginBottom: '3px' }}>
                      {err}
                    </li>
                  ))}
                </ul>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-slate)', padding: '2px', display: 'flex', flexShrink: 0 }}
              aria-label="Dismiss"
            >
              <X size={14} strokeWidth={2} />
            </button>
        ...
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {[0, 1, 2].map(i => (
              ...
              >
                <div style={{ height: '14px', background: 'var(--color-surface)', borderRadius: '3px', width: `${30 + i * 10}%`, marginBottom: '8px' }} />
                <div style={{ height: '12px', background: 'var(--color-surface)', borderRadius: '3px', width: `${45 + i * 8}%` }} />
              </div>
            ))}
          </div>
          ...
          <div style={{
            ...
            {!search && (
              <button
                ...
                onMouseEnter={e => e.currentTarget.style.background = '#b85d08'}
              ...
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {client.email && (
                      <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.8125rem', color: 'var(--color-slate)' }}>
                        {client.email}
                      </span>
                    )}
                    {client.phone && (
                ...
                <ChevronRight size={16} strokeWidth={2} style={{ color: 'var(--color-border)', flexShrink: 0 }} />
              </button>
            ))}
          </div>
        )}
        ...
        >
          ...
          >
            <div style={{ padding: '24px', overflowY: 'auto', flex: 1 }}>
              ...
              <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', color: 'var(--color-slate)', margin: '0 0 20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {mergeCandidates.map(mc => {
                  ...
                  return (
                    ...
                    >
                      <p style={{ margin: '0 0 2px', fontFamily: 'var(--font-body)', fontSize: '0.875rem', fontWeight: 600, color: 'var(--color-ink)' }}>
                        {mc.incoming.name}
                      </p>
                      <p style={{ margin: '0 0 12px', fontFamily: 'var(--font-body)', fontSize: '0.8125rem', color: 'var(--color-slate)' }}>
                        {mc.incoming.email} · existing: <strong style={{ color: 'var(--color-ink)', fontWeight: 600 }}>{mc.existing.name}</strong>
                      </p>
                        ...
                        {(['merge', 'separate'] as const).map(choice => (
                          <button
                            ...
                            style={{
                              ...
                              padding: '9px 12px',
                              borderRadius: 'var(--radius-md)',
                              border: resolution === choice ? 'none' : '1px solid var(--color-border)',
                              background: resolution === choice ? 'var(--color-primary)' : 'var(--color-card)',
                              color: resolution === choice ? '#fff' : 'var(--color-ink)',
                              fontFamily: 'var(--font-body)',
                              fontSize: '0.8125rem',
                              fontWeight: 600,
                              cursor: 'pointer',
                          ...
                          >
                            {choice === 'merge' ? 'Merge into existing' : 'Keep separate'}
                          </button>
                        ))}
                      </div>
                    </div>
