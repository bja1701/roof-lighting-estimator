...
import { Search, Plus, MapPin, Trash2, Loader2, Building2, Home, AlertTriangle, ArrowRight } from 'lucide-react';
...
function JobCardCover({ address, jobName, mapsApiKey }: { address: string | null; jobName: string; mapsApiKey: string }) {
  const [imgFailed, setImgFailed] = useState(false);
  const url =
    mapsApiKey && address?.trim()
      ? streetViewStaticImageUrl({ location: address.trim(), apiKey: mapsApiKey, width: 640, height: 360 })
      : '';
...
export default function JobsPage() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => { if (e.key === 'Escape') setJobPendingDelete(null); };
  ...
  const fetchJobs = async () => {
    ...
    if (data) setJobs(data.map((j: any) => ({ ...j, quote_count: j.quotes?.[0]?.count ?? 0 })));
    setLoading(false);
  };
  ...
  const openDeleteConfirm  = (job: Job, e: React.MouseEvent) => { e.preventDefault(); e.stopPropagation(); setJobPendingDelete(job); };
  const cancelDeleteConfirm = () => { if (deletingId) return; setJobPendingDelete(null); };
      ...
      <div className="px-4 sm:px-6 md:px-8 py-8 max-w-6xl mx-auto">
        ...
        {estimateLimitBanner && (
          <div
            ...
            style={{ background: '#fff8f0', border: '1px solid var(--color-warning)' }}
          >
            <div className="flex items-start gap-3">
              <AlertTriangle size={18} style={{ color: 'var(--color-warning)', flexShrink: 0, marginTop: 1 }} />
              <p className="text-sm" style={{ color: 'var(--color-ink)' }}>
                You've used all <strong>5</strong> free estimates. Upgrade to unlock the estimator.
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="mb-6 relative">
          <Search size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2" style={{ color: 'var(--color-slate)' }} />
          <input
            type="text"
        ...
        {loading ? (
            ...
            {[1, 2, 3].map(i => (
              <div key={i} className="rounded-xl overflow-hidden animate-pulse" style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
                ...
                <div className="p-4 space-y-2.5">
                  ...
                  <div className="h-3 rounded" style={{ background: 'var(--color-surface)', width: '50%' }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 rounded-2xl" style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
            ...
            <h3 className="font-semibold text-lg mb-1" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-ink)' }}>
              {search ? 'No matching jobs' : 'No jobs yet'}
            </h3>
            <p className="text-sm mb-6" style={{ color: 'var(--color-slate)' }}>
              {search ? 'Try a different search term' : 'Create your first job to start estimating rooflines'}
            </p>
            {!search && (
              <button
                onClick={() => setShowNewJob(true)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {filtered.map(job => (
              ...
              >
                {/* Thumbnail */}
                <div className="relative h-28 w-full overflow-hidden" style={{ background: 'var(--color-surface)' }}>
                ...
                {/* Card body */}
                <div className="p-4">
                  <h3 className="font-semibold text-base leading-snug mb-1" style={{ fontFamily: 'var(--font-display)', color: 'var(--color-ink)' }}>
                    {job.name}
                  </h3>
                  {job.address && (
                    <p className="text-xs flex items-center gap-1 mb-3" style={{ color: 'var(--color-slate)' }}>
                  ...
                  <div className="flex items-center justify-between pt-3" style={{ borderTop: '1px solid var(--color-border)' }}>
                    <div className="flex items-center gap-4">
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: 'var(--color-slate)' }}>Created</p>
                        <p className="text-xs font-semibold" style={{ color: 'var(--color-ink)' }}>
                          {new Date(job.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                        </p>
                      </div>
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: 'var(--color-slate)' }}>Estimates</p>
                        <p className="text-xs font-semibold" style={{ color: 'var(--color-ink)' }}>{job.quote_count ?? 0}</p>
                      </div>
                    </div>
                    <ArrowRight size={16} style={{ color: 'var(--color-primary)', transition: 'transform 150ms ease-out' }}
        ...
        >
          <div className="w-full max-w-md overflow-hidden rounded-xl" style={{ background: 'var(--color-card)', boxShadow: 'var(--shadow-modal)' }}>
            <div className="p-6">
              ...
              <p className="mb-2 text-sm" style={{ color: 'var(--color-slate)' }}>
                <span className="font-semibold" style={{ color: 'var(--color-ink)' }}>"{jobPendingDelete.name}"</span> will be removed permanently.
              </p>
              {(jobPendingDelete.quote_count ?? 0) > 0 ? (
                <p className="mb-5 text-sm" style={{ color: 'var(--color-destructive)' }}>
                  This also deletes {jobPendingDelete.quote_count} saved estimate{(jobPendingDelete.quote_count ?? 0) === 1 ? '' : 's'}.
                </p>
              ) : (
                <p className="mb-5 text-sm" style={{ color: 'var(--color-slate)' }}>This cannot be undone.</p>
              )}
              <div className="flex gap-3">
                ...
                <button
                  ...
                  onMouseLeave={e => (e.currentTarget.style.opacity = '1')}
                >
                  {deletingId ? <><Loader2 size={15} className="animate-spin" /> Deleting…</> : <><Trash2 size={15} /> Delete job</>}
                </button>
              </div>
            </div>
          </div>
