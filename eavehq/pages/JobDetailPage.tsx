...
export default function JobDetailPage() {
	...
	const handleSendEstimateOptions = async () => {
		...
		if (Number.isNaN(parsedPct) || parsedPct < 1 || parsedPct > 100) {
		...
		try {
			...
			const res = await fetch(SEND_ESTIMATE_OPTIONS_URL, {
				...
				body: JSON.stringify({
					...
					...(sendOptionsMessage.trim()
						? { custom_message: sendOptionsMessage.trim() }
						: {}),
				}),
			});
			if (!res.ok) {
			...
			await supabase
				.from('jobs')
				.update({ deposit_percent: parsedPct })
				.eq('id', id);
			...
			setSendOptionsError(
				err instanceof Error ? err.message : 'Failed to send estimate options'
			);
		} finally {
	...
	const handleAdvanceStatus = async () => {
		...
		const isStartedTransition =
			job.status === 'scheduled' && nextStatus === 'in_progress';
		...
		if (
			...
			setConfirmDialog({
				...
				message:
					'No deposit has been recorded for this job. Mark as Scheduled anyway?',
	...
	const handleOpenEstimator = (quote?: Quote) => {
		...
		if (quote?.canvas_state) {
			...
			if (job?.address && !state.estimateSiteAddress)
				state.estimateSiteAddress = job.address;
	...
	if (loading) {
		return (
			<SharedLayout>
				<div
					className="flex items-center justify-center gap-2 py-32"
	...
	const cfg = JOB_STATUS_CONFIG[job.status];
	const showAdvance =
		cfg.nextManualStatus &&
		cfg.nextManualLabel &&
		!(
			job.status === 'scheduled' &&
			cfg.nextManualStatus === 'in_progress' &&
			quotes.length === 0
		);
		...
		<SharedLayout>
			<div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 pb-20 pt-8">
				...
				{job.address ? (
					...
					>
						...
						<div
							...
							style={{
								background:
									'linear-gradient(90deg, rgba(31,61,44,0.92) 0%, rgba(31,61,44,0.72) 48%, rgba(31,61,44,0.18) 100%)',
							}}
						/>
						<div className="relative flex min-h-[220px] max-w-2xl flex-col justify-end p-5 sm:p-6">
							...
							<div
								...
								style={{ color: 'rgba(247,243,234,0.84)' }}
							>
				...
				<div className="flex flex-wrap gap-2 items-center mb-4">
					...
					{job.estimate_sent_at != null && (
						...
						>
							<MailCheck size={12} />
							Sent{' '}
							{new Date(job.estimate_sent_at).toLocaleDateString('en-US', {
								month: 'short',
								day: 'numeric',
								year: 'numeric',
							})}
					...
					{job.estimate_sent_at != null && (
						<span
							className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full"
							style={{
								background: 'rgba(217,111,10,0.1)',
								border: '1px solid rgba(217,111,10,0.22)',
								color: 'var(--color-accent)',
							}}
						>
							<Send size={12} />
							{job.followup_count ?? 0} follow-up
							{(job.followup_count ?? 0) === 1 ? '' : 's'}
						</span>
					)}
					{job.deposit_paid_at != null && (
						...
						>
							<CheckCircle size={12} />
							Deposit{' '}
							{new Date(job.deposit_paid_at).toLocaleDateString('en-US', {
								month: 'short',
								day: 'numeric',
							})}
							{job.deposit_amount != null &&
					...
					{job.final_paid_at != null && (
						...
						>
							<CheckCircle size={12} />
							Final paid{' '}
							{new Date(job.final_paid_at).toLocaleDateString('en-US', {
								month: 'short',
								day: 'numeric',
							})}
				...
				<div className="flex items-center justify-between mb-5">
					...
					<button
						...
						onMouseEnter={(e) => {
							if (canEstimate) e.currentTarget.style.opacity = '0.88';
						}}
						onMouseLeave={(e) => (e.currentTarget.style.opacity = '1')}
					>
						<Plus size={15} />
							...
							<QuoteCard
								...
								onDiscountChange={(qId, amount, type) =>
									setQuotes((qs) =>
										qs.map((q) =>
											q.id === qId
												? { ...q, discount_amount: amount, discount_type: type }
												: q
										)
									)
								}
				...
				>
					...
					<button
						...
						onMouseEnter={(e) => {
							e.currentTarget.style.background = 'rgba(201,64,64,0.08)';
						}}
						onMouseLeave={(e) => {
						...
						<div className="px-6 pt-6 pb-5">
							...
							>
								...
								<span
									...
									style={{ color: 'var(--color-ink)' }}
								>
									{quotes.length} estimate{quotes.length !== 1 ? 's' : ''}
								</span>{' '}
								to{' '}
								<span
									className="font-semibold"
							...
							<div
								className="rounded-lg px-4 py-3 mb-4 space-y-1"
								style={{
									background: 'var(--color-surface)',
									border: '1px solid var(--color-border)',
								}}
							>
								{quotes.map((q) => (
									...
									>
										{q.label}
										{q.total_price != null
											? ` — $${q.total_price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
											: ''}
									</p>
								))}
							</div>
			...
			{confirmDialog && (
				<div
					...
					aria-modal="true"
					onClick={(e) =>
						e.target === e.currentTarget && setConfirmDialog(null)
					}
				>
					<div
